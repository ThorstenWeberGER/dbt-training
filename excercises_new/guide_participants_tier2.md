...
### Task 2 — Debug `models/3_gold/mrt_country_summary.sql`

A colleague has submitted `models/3_gold/mrt_country_summary.sql` for review. Run the model and inspect the output:

```bash
dbt run --select mrt_country_summary
dbt show --select mrt_country_summary --limit 10
```

**What do you notice?** Look at the `contact_count`, `deal_count`, and `total_deal_amount` columns.

All counts are 0. Something is wrong with the join. Open `models/3_gold/mrt_country_summary.sql` and find the bug.

<details>
<summary>Hint</summary>

Look at the `LEFT JOIN` between `country_codes` and `dim_patient`. The join condition compares `cc.country_code` to a column on `dim_patient`. Are you sure that column contains country codes?
...
The bug is on the join line:
```sql
-- Bug: comparing a 2-letter country code to an MD5 surrogate key hash
LEFT JOIN {{ ref('dim_patient') }} AS p  ON cc.country_code = p.patient_key

-- Fix: join on the actual country_code column
LEFT JOIN {{ ref('dim_patient') }} AS p  ON cc.country_code = p.country_code
```

After fixing, run again. You should see Germany (DE) with 8 contacts, the highest count.
...
```bash
dbt run --select mrt_country_summary
dbt show --select mrt_country_summary --limit 5
```
Expected: `contact_count > 0` for DE, FR, GB, IT, DK.
...
### Bonus — Rename output tables with `+alias`

The seeds in this project have names like `raw_contacts.csv` but the source tables they simulate are called `contacts`. Open `dbt_project.yml` and find the seed section.

**Question:** How does dbt know to load the seed as `contacts` instead of `raw_contacts`?

<details>
<summary>Answer</summary>

The `+alias` key changes the name of the resulting database object without changing the filename:
...
seeds:
  dbt_training:
    raw_contacts:
      +schema: hubspot_raw
      +alias: contacts       # the seed lands as hubspot_raw.contacts
```

`ref('raw_contacts')` still resolves using the **filename** (node name). `+alias` only affects what appears in the database. This is why `source('hubspot', 'contacts')` in staging models finds the seed.

</details>

**Three practical uses of `+alias` to know:**

| Use case | Example |
|---|---|
| Fix a name mismatch | Seed file is `raw_contacts.csv` but the source expects `contacts` — alias solves it without renaming the file |
| Point to a smaller dev table | In development, alias a 1 000-row sample instead of the full 10 M-row table to keep runs fast |
| Incremental testing | Alias to `table_v1`, run; alias to `table_v2` (added/changed rows), run again — verify the incremental model picks up only the delta |

**Try it:** In `dbt_project.yml`, change the `+alias` for `raw_contacts` from `contacts` to `contacts_test`, run `dbt seed --select raw_contacts --target test`, then run `dbt run --select stg_hubspot__contacts --target test`. What error do you get and why?
...
Open any two Silver models (`dim_patient.sql`, `fct_prescription.sql`, or `fct_deal.sql`). Find one CTE or expression that repeats across models and answer:
...
> **Context:** `stg_hubspot__contacts` is already built. The snapshot file `snapshots/snap_patients.sql` has been pre-built for you as a scaffold. Your job is to understand the config, run it, and observe SCD2 in action.
>
> Snapshots live in the `snapshots/` folder (not `models/`). They are run with `dbt snapshot`, not `dbt run`.

---

### Task 1 — Review `snapshots/snap_patients.sql`

Open the file `snapshots/snap_patients.sql`. Read it carefully and answer:

1. What is the `unique_key`?
2. What strategy is used? What does it require from the source data?
3. Which column triggers a new SCD2 row when it changes?
4. Which columns are tracked in the snapshot?

<details>
<summary>Answers</summary>

The file tracks patients (HubSpot contacts) with these settings:
- `unique_key = 'contact_id'` — one snapshot row per patient
- `strategy = 'timestamp'` — dbt compares `updated_at` on every run
- `updated_at = 'updated_at'` — a newer timestamp triggers a new SCD2 row
- Columns tracked: `contact_id`, `first_name`, `last_name`, `email`, `country_code`, `pipeline_stage_id`, `created_at`, `updated_at`
...
# DuckDB local testing (run dbt seed first if not already done):
dbt seed --target test
dbt snapshot --target test
```

Expected terminal output: `Created snapshot snapshots.snap_patients`

Verify the snapshot was created:

```bash
dbt show --select snap_patients --limit 10
```

Or query directly:
```sql
SELECT COUNT(*) FROM snapshots.snap_patients;
SELECT contact_id, email, dbt_valid_from, dbt_valid_to FROM snapshots.snap_patients LIMIT 5;
```

**Expected:**
- 20 rows (one per patient/contact)
- All rows have `dbt_valid_to = NULL` (every patient is current — this is the initial load)
- All rows share the same `dbt_valid_from` timestamp
...
**Step 1 — Simulate a data change**

*Snowflake:* Run an UPDATE on the `HUBSPOT_RAW.contacts` table:
```sql
UPDATE DBT_TRAINING.HUBSPOT_RAW.contacts
SET email = 'new.email@example.com', updated_at = '2024-05-11 08:00:00'
WHERE id = 3;
```

*DuckDB local testing:* Edit `seeds/raw_contacts.csv` — find `contact_id = 3`. Change their `email` to any new address. Bump `updated_at` by one day (e.g., from `2024-05-07 16:45:00` to `2024-05-08 16:45:00`).

**Step 2 — Reload the source data** (DuckDB only — Snowflake skip this)

```bash
dbt seed --target test --full-refresh --select raw_contacts
```

This reloads the changed CSV into DuckDB. Without this, the snapshot sees no changes.
...
dbt detects the change in `contact_id = 3` (the `updated_at` value is newer than what the snapshot stored) and inserts a new SCD2 row.
...
Write a SQL query that answers: **"What email address did patient 3 have on 2024-06-01?"**

Use only the snapshot table. Do not look at the source data.

Requirements:
- Returns exactly one row
- Uses `dbt_valid_from` and `dbt_valid_to` to find the active version on that date
...
This returns exactly one row: the version of patient 3 that was active on June 1, 2024.
...
stg_hubspot__owners ────┤
                        ├─► dim_patient ──┐
                        └─► dim_doctor  ──┤─► fct_prescription ──► mrt_patient_prescriptions
stg_prescriptions ──────────────────────┘
...
The prefix `+` walks upstream in the DAG. This selects `stg_hubspot__contacts`, `stg_prescriptions`, `dim_patient`, `dim_doctor`, `dim_product`, and `fct_prescription` — the full upstream chain.
