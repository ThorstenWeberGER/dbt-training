...
# Setting your project in Snowflake

## Create dbt project

> In this excercise we will use a existing repo. So we can directly download this from Git.

**Task:** Create new workspace from this Git repo: https://github.com/ThorstenWeberGER/dbt-training-excercise

**Task 2**: Create your personal branch, like `dbt_tweber`

## Configure Your Profile

**Task**: Open the profiles.yml and fill in the required fields.

| Key         | Example                  | Required <br>locally | Required in<br>Snowflake |
| ----------- | ------------------------ | -------------------- | ------------------------ |
| `type`      | `snowflake`              | x                    | x                        |
| `account`   | `xy12345.eu-west-1`      | x                    |                          |
| `user`      | `jane@bloomwell.com`     | x                    | x                        |
| `role`      | `analytics_service_role` | x                    | x                        |
| `warehouse` | `dbt_wh`                 | x                    | x                        |
| `database`  | `bloomwell_staging`      | x                    | x                        |
| `schema`    | `dev_jane`               | x                    | x                        |
| `threads`   | `8`                      |                      |                          |

### Extra tip: Working locally you need to store a token
----

> **Never put secrets into `profiles.yml`.** Instead create your secret, a .secrets folder, .gitignore it and then refer to it.

```
      user: jane@BLOOMWELL.DE
      private_key_path: .secrets/rsa_key.p8
```

## Setup a .gitignore

**Task**: protect following folders from being synced to github. They either contain secrets or runtime artefacts and more. Even if they are not present yet, they eventually will.
```
general
.secrets/          # if you work locally
.snowflake/     # if you work locally
.claude/          # if you work locally
logs/
temp/
.venv/              # if you work locally
...
----
### Commit and push
**Task:** Save your edits. Commit your edits to your **personal branch**.
...
Choose `dbt build` and take a look at the console output. Browse through the output. What do you find? Where do you find `done`. Where do you find `warning`or `error`. What else is there.

Paste the error message into *Claude* and ask for help to fix a dbt model. What i the advice?
...
**Prerequisite:** `profiles.yml` configured
...
Open `target` folder and look for the materialized model `stg_hubspot__contacts.sql`.
